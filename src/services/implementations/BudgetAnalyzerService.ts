...
export class BudgetAnalyzerService implements IBudgetAnalyzerService {
    ...
    private checkSet(products: Product[]): void {
        const listPrice: number = products.reduce<number>((sum: number, product: Product) => {
            sum = sum + product.price;

            return sum;
        }, 0);

        const listPriority: number = products.reduce<number>((sum: number, product: Product) => {
        ...
        } else if (listPrice <= this.bestPriceSum && listPriority > this.bestPrioritySum) {
            this.bestChoice = products;
            this.bestPrioritySum = listPriority;
        }
    }
